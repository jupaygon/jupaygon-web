...
draft: false
description: "Addy Osmani and Boris Cherny gave a name to something I'd been building for months: you stop prompting the agent and start designing the loop that prompts it. Here are its six pieces mapped onto my own setup, and the bill nobody puts on the slide."
...
In June 2026 Addy Osmani and Boris Cherny put a name on something a few of us had already been doing quietly for months: **loop engineering**.
...
I'd been building exactly that without the label. So this post is two things at once: a map of the six pieces Osmani lays out, each one wired to my real setup, and an honest note about the part nobody puts on the slide: the bill.
...
Osmani breaks the loop into six pieces. I'll take them in order.

## 1. Automations: the heartbeat

A loop needs something that fires without me. Scheduled jobs and watchers that discover work and surface it instead of waiting for me to type.
...
I keep a versioned set of them. One pins down the API surface of an admin framework that broke half its methods between major versions, so the agent stops guessing. Another encodes a technical-SEO checklist. They live in the repo, reviewed like code, because [that's what they are](/en/posts/2026-03-29/ai-skills-easyadmin5/): executable intent, not documentation that rots in a wiki.
...
This is the difference between "here's what I would do" and "done, here's the link". It's also where a [routing table](/en/posts/2026-05-13/determinism-in-ai-workflows/) earns its keep: the loop must reach a private PR through the GitHub connector, never through `curl`, or it loses identity, logging and auth handling in one shortcut.
...
But the leverage cuts both ways. A well-designed loop multiplies a careful engineer. It also multiplies a careless one, faster. The design work, the part that's actually engineering, isn't wiring up the six pieces. It's deciding what the loop is allowed to do on its own, and keeping the one checkpoint that's still mine: shipping code I confirmed works, and understand.
