...
draft: false
description: 'Addy Osmani y Boris Cherny le pusieron nombre a algo que yo llevaba meses construyendo: dejas de promptear al agente y empiezas a diseñar el bucle que lo promptea. Aquí están sus seis piezas mapeadas sobre mi propio setup, y la factura que nadie pone en la diapositiva.'
...
En junio de 2026, Addy Osmani y Boris Cherny le pusieron nombre a algo que unos cuantos llevábamos meses haciendo en silencio: **loop engineering**.
...
Yo llevaba meses construyendo justo eso sin la etiqueta. Así que este post es dos cosas a la vez: un mapa de las seis piezas que describe Osmani, cada una conectada a mi setup real, y una nota honesta sobre la parte que nadie pone en la diapositiva: la factura.
...
Osmani parte el bucle en seis piezas. Las tomo en orden.

## 1. Automatizaciones: el latido

Un bucle necesita algo que dispare sin mí. Tareas programadas y vigilantes que descubren trabajo y lo sacan a la superficie en vez de esperar a que yo escriba.
...
Tengo un conjunto versionado de ellas. Una fija la superficie de API de un framework de administración que rompió la mitad de sus métodos entre versiones mayores, para que el agente deje de adivinar. Otra codifica un checklist de SEO técnico. Viven en el repo, revisadas como código, porque [eso es lo que son](/es/posts/2026-03-29/ai-skills-easyadmin5/): intención ejecutable, no documentación que se pudre en una wiki.
...
Es la diferencia entre "esto es lo que haría" y "hecho, aquí tienes el enlace". Y es donde una [tabla de routing](/es/posts/2026-05-13/determinism-in-ai-workflows/) se gana el sueldo: el bucle tiene que llegar a una PR privada por el conector de GitHub, nunca por `curl`, o pierde identidad, logging y manejo de auth en un solo atajo.
...
Pero la palanca corta por los dos lados. Un bucle bien diseñado multiplica a un ingeniero cuidadoso. También multiplica a uno descuidado, más rápido. El trabajo de diseño, la parte que de verdad es ingeniería, no es cablear las seis piezas. Es decidir qué se le permite hacer al bucle por su cuenta, y conservar el único punto de control que sigue siendo mío: entregar código que verifiqué que funciona, y que entiendo.
